...
def aplicar_bloque9(df, ruta_gadm, idioma=None, usar_nominatim=True):
    ...
    for idx in registros_procesar:
        ...
        pais   = str(row.get(cols["pais"], "Colombia")).strip()
        ...
        if lat is not None and lon is not None:
            df.at[idx, cols["lat_final"]]  = lat
            df.at[idx, cols["lon_final"]]  = lon
    ...
    df.loc[nivel1_mask, cols["lat_final"]] = df.loc[nivel1_mask, "lat_wgs84"]
    df.loc[nivel1_mask, cols["lon_final"]] = df.loc[nivel1_mask, "lon_wgs84"]
