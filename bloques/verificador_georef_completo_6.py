...
def preparar_coordenadas(df, idioma):
    ...
    lats = df.loc[mask, col_lat_o].apply(
        lambda x: (f"{float(corregir_coordenada(x, es_lon=False)):.6f}"
                   if corregir_coordenada(x, es_lon=False) is not None else "")
    )
    lons = df.loc[mask, col_lon_o].apply(
        lambda x: (f"{float(corregir_coordenada(x, es_lon=True)):.6f}"
                   if corregir_coordenada(x, es_lon=True) is not None else "")
    )
    ...
    if 'lat_wgs84' in df.columns:
        mask_b8 = df['lat_wgs84'].notna() & (df['lat_wgs84'].astype(str).str.strip() != '') & (df['lat_wgs84'].astype(str) != 'nan')
        if mask_b8.any():
            b8_lats = df.loc[mask_b8, 'lat_wgs84'].apply(
                lambda x: f"{float(x):.6f}" if pd.notna(x) and str(x) not in ('','nan') else "")
            b8_lons = df.loc[mask_b8, 'lon_wgs84'].apply(
                lambda x: f"{float(x):.6f}" if pd.notna(x) and str(x) not in ('','nan') else "")
            df.loc[mask_b8, col_lat_f] = b8_lats.astype(str)
            df.loc[mask_b8, col_lon_f] = b8_lons.astype(str)
    return df
...
def aplicar_bloque10(df, ruta_salida=None, idioma=None):
    ...
    COLS_INTERNAS = {
        ...
        "conversion_nota", "rango_colombia", "conversion_estado",
        "lat_decimal_calculada", "lon_decimal_calculada",
        "elevacion_api", "elevacion_fuente", "elevacion_estado", "elevacion_nota",
        "B6_estado_localidad", "B6_caso", "B6_nota_localidad",
    }
    ...
    # Convertir TODO el DataFrame a string puro antes de exportar
    # Solución definitiva para "Invalid value for dtype str/float"
    import numpy as np
